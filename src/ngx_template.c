    ...
    ngx_str_t template)
{
    ...
    ngx_str_t     key, val, s;
    u_char       *c1, *c2, *c3, *end = template.data + template.len;
    ...
    for (c1 = ngx_strnstr(c2, "{{", end - c2);
         c1 != NULL;
         c1 = ngx_strnstr(c2, "{{", end - c2)) {
        ...
        c3 = c1 + 2;
        c2 = ngx_strnstr(c3, "}}", end - c3);

        do {
            c3 = ngx_strnstr(c3, "{{", c2 - c3);
            if (c3 == NULL)
                break;
            if (c3 < c2) {
                c3 += 2;
                c2 = ngx_strnstr(c2 + 2, "}}", end - c2 - 2);
                if (c2 == NULL)
                    break;
            }
        } while (1);
        ...
        do {
            k = ngx_split(k.key, '|');
            ngx_trim(&k.key);
            val = lookup_key(cycle, conf, transform(cycle, pool, conf, k.key));
            if (val.data != NULL)
            ...
            case YAML_SCALAR_EVENT:
                ...
                if (level == 1) {
                    ...
                    if (ngx_memn2cmp(t->group.data, event.data.scalar.value,
                        ...
                        if (rc != NGX_OK)
                            goto done;
                        type = YAML_STREAM_END_EVENT;
                    } else {
                        s.data = event.data.scalar.value;
                        s.len = event.data.scalar.length;
